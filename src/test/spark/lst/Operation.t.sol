...
contract SparkLSTOperationTest is SetupSparkLST, OperationTest {
    function setUp() public override(SetupSparkLST, OperationTest) {
        SetupSparkLST.setUp();
    }
    ...
    function _maxUnwindCollateralDust(
        ...
        uint256 relativeDust = collateralBeforeUnwind / 100;
        return
    ...
    function _createUnderLeveragedPositionSpark(uint256 equity) internal {
        vm.prank(management);
        ...
        override
    {
        ...
        _createUnderLeveragedPositionSpark(equity);
    ...
    function test_sparkContractResolution() public {
        AaveLooper looper = AaveLooper(payable(address(strategy)));

        assertEq(looper.POOL(), SPARK_POOL, "!spark pool");
        assertTrue(
            address(looper.DATA_PROVIDER()) != address(0),
            "!data provider"
        );
        assertTrue(address(looper.AAVE_ORACLE()) != address(0), "!oracle");
        assertTrue(
            address(looper.REWARDS_CONTROLLER()) != address(0),
            "!rewards"
        );
        assertTrue(looper.A_TOKEN() != address(0), "!aToken");
        assertTrue(looper.VARIABLE_DEBT_TOKEN() != address(0), "!debt token");
    }
